...
class RNN_model(nn.Module):
    def __init__(self, batch_sz ,vocab_len ,word_embedding,embedding_dim, lstm_hidden_dim):
        ...
        # ???
        self.num_layers=2
        #print('lstm_dim ',lstm_hidden_dim)
        #print('batch_size',batch_size)
        self.lstm=nn.LSTM(self.word_embedding_dim,self.lstm_dim,num_layers=2)
    ...
    def forward(self,sentence,is_test = False,hidden=None):
        ...
        # ???
        #batch_size=sentence.shape
        #print('batch_input',batch_input.shape)
        c,batch_size,a=batch_input.shape
        if hidden is None:
            h_0 = torch.zeros(self.num_layers * 1, batch_size, self.lstm_dim)#batch_size
            #print('h_0.shape',h_0.shape)
            c_0 = torch.zeros(self.num_layers * 1, batch_size, self.lstm_dim)
            #print('c_0.shape',c_0.shape)
        else:
            h_0, c_0 = hidden
        output, hidden = self.lstm(batch_input, (h_0, c_0))  # hidden 是h,和c 这两个隐状态
        ################################################
...
def run_training():
    ...
    print('poems_vector',poems_vector)
    print('word_to_int',word_to_int)
    ...
    word_embedding1 = word_embedding( vocab_length= len(word_to_int) + 1 , embedding_dim= 100)
    rnn_model = RNN_model(batch_sz = BATCH_SIZE,vocab_len = len(word_to_int) + 1 ,word_embedding = word_embedding1 ,embedding_dim= 100, lstm_hidden_dim=128)
...
def gen_poem(begin_word):
    ...
    word_embedding1 = word_embedding(vocab_length=len(word_int_map) + 1, embedding_dim=100)
    rnn_model = RNN_model(batch_sz=100, vocab_len=len(word_int_map) + 1, word_embedding=word_embedding1,
