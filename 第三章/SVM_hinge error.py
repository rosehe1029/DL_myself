...
def calcEk(oS, k): #计算Ek（参考《统计学习方法》p127公式7.105）
    ...
    #print('oS.labelMat.shape',oS.labelMat.shape)
    #print('oS.K[:,k].shape',oS.K[:,k].shape)
    #fxi=float(np.multiply(alphas,label_mat).T*(data_matrix*data_matrix[i,:].T))+b
    #Ei=fxi-float(label_mat[i])
    fXk = (multiply(oS.alphas,oS.labelMat).T*oS.K[:,k] + oS.b).astype(np.float)
    #print('fXk.shape',fXk.shape)
...
def testB(data_train,data_test):
    ...
    for i in range(m):
        ...
        if sign(predict)==sign(labelArr[i]): #sign函数 -1 if x < 0, 0 if x==0, 1 if x > 0
            errorCount += 1
    print("the training true rate is: %f" % (float(errorCount)/m)) #打印出错误率
    ...
    for i in range(m): #在测试数据上检验错误率
        ...
        if sign(predict)==sign(labelArr_test[i]):
            errorCount_test += 1
    print("the test true rate is: %f" % (float(errorCount_test)/m))
    #acc_test = eval_acc(labelArr_test, predict)
